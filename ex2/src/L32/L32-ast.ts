...
;;         |  ( dict ( symbol <cexp>)*)		/ DictExp(entries: pair[])
;; <pair>     ::= <symbol> <cexp>             / Pair(key: string, val:Cexp)
...
export type Pair = { tag: "Pair"; key: string; val: CExp };
export type DictExp = { tag: "DictExp"; entries: Pair[] };
...
export const makePair = (key: string, val: CExp): Pair =>
    ({ tag: "Pair", key, val });

export const makeDictExp = (entries: Pair[]): DictExp =>
    ({ tag: "DictExp", entries });
...
const parsePair = (sexp: Sexp): Result<Pair> =>
    isCompoundSexp(sexp) && isNonEmptyList<Sexp>(sexp) && sexp.length === 2 ?
        (isString(first(sexp)) ? 
            mapv(parseL32CExp(second(sexp)), (val: CExp) =>
                makePair(first(sexp) as string, val)) :
            makeFailure(`Invalid dict key, expected symbol: ${format(first(sexp))}`)) :
    makeFailure(`Invalid dict pair format: ${format(sexp)}`);
